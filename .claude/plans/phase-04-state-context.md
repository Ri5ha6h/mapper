# Phase 04: State Context

**Creates:** `lib/mapper/context.tsx`
...
React context for global mapper state: source/target files, mappings, expanded nodes.
...
export function MapperProvider({ children }: { children: ReactNode }) {
  ...
  const setSource = useCallback((data: FileData | null) => {
    dispatch({ type: "SET_SOURCE", payload: data })
  }, [])

  const setTarget = useCallback((data: FileData | null) => {
...
```tsx
// In component
const { source, mappings, addMapping, toggleExpand } = useMapper();
```
...
- [x] Source/target set correctly
- [x] Mappings cleared on file change
- [x] Duplicate mappings prevented
- [x] Toggle expand works
