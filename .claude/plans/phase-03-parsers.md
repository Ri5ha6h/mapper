...
**Creates:** `lib/mapper/parsers.ts`

**Depends on:** Phase 02 (types)

---

## Overview

Parse JSON and XML content into TreeNode structure. Auto-detect file type.
...
```typescript
// JSON
parseJSON('{"user": {"name": "John", "age": 30}}');
// → TreeNode with id="root", children=[{id="root.user", children=[...]}]

// XML
parseXML('<person id="1"><name>John</name></person>');
// → TreeNode with children=[{id="root.person.@id"}, {id="root.person.name"}]
```
...
- [x] `detectFileType` correctly identifies JSON vs XML
- [x] JSON arrays produce `[n]` indices
- [x] XML attributes prefixed with `@`
- [x] Nested structures preserve correct depth
- [x] Empty objects/arrays handled
